...
class PDF extends FPDF {
    ...
    function Header() {
        ...
        $this->Cell(90, 10, 'Name', 1); // Width increased by 5
        $this->Cell(40, 10, 'Phone', 1); 
        $this->Cell(85, 10, 'Email', 1); // Width increased by 10
        $this->Ln();
    }
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        $pdf->MultiCell(90, 10, htmlspecialchars($row['name']), 1); // Width increased by 5
        $pdf->SetXY($xPos + 90, $yPos); // Move to the right after MultiCell

        $pdf->Cell(40, 10, htmlspecialchars($row['phone']), 1); 

        $xPos = $pdf->GetX(); // Save the current X position
        $yPos = $pdf->GetY(); // Save the current Y position
        $pdf->MultiCell(85, 10, htmlspecialchars($row['email']), 1); // Width increased by 10
